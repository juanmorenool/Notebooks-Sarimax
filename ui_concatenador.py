...
def render_concatenador(NAVY, BLUE, GREEN, RED, GRAY, LTGRAY, TEXT, MUTED, BG, WHITE, BORDER, TINT):
    """
    Renderiza la interfaz del concatenador como un tab/sección.
    ...
    <div style="margin-bottom:4px;">
        ...
        <p style="font-size:12px;color:{MUTED};margin:4px 0 0;">Carga múltiples archivos CSV de impacto y obtén un único archivo consolidado.</p>
    </div>
    ...
    with col1:
        st.markdown(section_title("Configuración"), unsafe_allow_html=True)
        pais = st.selectbox(
            "País",
            options=["CO", "PA", "CR"],
    ...
    with col2:
        ...
        <div style="background:#F0F7FF;border:1px solid #B3D9FF;border-radius:6px;padding:12px;margin-bottom:12px;">
            <p style="font-size:11px;color:#004494;margin:0;"><b>Patrón esperado:</b></p>
    ...
    if archivos_cargados:
        ...
        # --- Mostrar avisos ---
        if avisos:
            st.markdown(section_title("ℹ️ Avisos"), unsafe_allow_html=True)
            for aviso in avisos:
                st.info(aviso)
        ...
        if df_resultado is not None and not df_resultado.empty:
            ...
            st.markdown(divider(), unsafe_allow_html=True)
            st.markdown(section_title("Estadísticas de variables de impacto"), unsafe_allow_html=True)
        ...
        <div style="background:{TINT};border:1px solid {BORDER};border-radius:8px;padding:40px;text-align:center;margin-top:20px;">
            <p style="font-size:13px;color:{NAVY};font-weight:600;margin:0 0 6px;">Ningún archivo seleccionado</p>
            <p style="font-size:11px;color:{MUTED};margin:0;">Sube uno o más archivos Impacto_*.csv para comenzar.</p>
        </div>
        """, unsafe_allow_html=True)
